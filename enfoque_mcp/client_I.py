...
async def run_agent(pdf_path):
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # Creamos un agente con tool-calling correcto
            agent = create_react_agent(model, tools)

            print("🤖 Solicitando extracción al agente...")

            # Instrucción natural pero explícita en herramientas
            instruction = f"""
            ...
            No expliques nada más. Solo actúa invocando a tools para posterior ejecución.
            """
